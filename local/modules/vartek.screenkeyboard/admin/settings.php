...
$APPLICATION->SetTitle(Loc::getMessage("VSK_SETTINGS_MAIN"));

if ($request->isPost() && $request->getPost("Update") && check_bitrix_sessid()) {
    Option::set($moduleId, "auto_enable", $request->getPost("auto_enable") === "Y" ? "Y" : "N");
    Option::set($moduleId, "theme",       $request->getPost("theme"));
    Option::set($moduleId, "position",    $request->getPost("position"));
    Option::set($moduleId, "animation",   $request->getPost("animation") === "Y" ? "Y" : "N");
    ...
    <form method="post" action="<?= $APPLICATION->GetCurPage() ?>?mid=<?= $moduleId ?>&lang=<?= LANGUAGE_ID ?>">
        ...
        <table class="adm-detail-content-table edit-table">
            ...
            <tr>
                ...
                <td width="60%">
                    <input type="checkbox" name="auto_enable" value="Y"
                        <?= $settings["auto_enable"] === "Y" ? "checked" : "" ?>>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="checkbox" name="animation" value="Y"
                        <?= $settings["animation"] === "Y" ? "checked" : "" ?>>
                </td>
            </tr>
        </table>
